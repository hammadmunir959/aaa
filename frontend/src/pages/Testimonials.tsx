...
const Testimonials = () => {
  ...
  const getStatusBadge = (status: Testimonial["status"]) => {
    switch (status) {
      case "pending":
        return (
          <Badge className="bg-gold/20 text-gold hover:bg-gold/30 border-gold/40">
            Pending
          </Badge>
        );
      case "approved":
  ...
  const renderRating = (rating: number) => (
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, index) => (
        <Star
          key={index}
          className={`w-4 h-4 ${index < rating ? "fill-yellow-500 text-gold" : "fill-gray-200 text-gray-200"
            }`}
        />
      ))}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="bg-white dark:bg-card shadow rounded-xl mb-6">
            <div className="flex flex-col sm:flex-row justify-between items-center p-4 gap-4">
              <Button onClick={handleAddTestimonial} className="w-full sm:w-auto bg-gold hover:bg-gold/90 text-black font-bold">
