  ...
  const onSubmit = async (data: AdminRegisterFormData) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.data?.details) {
        ...
        Object.entries(error.data.details).forEach(([key, messages]) => {
          if (Array.isArray(messages) && messages.length > 0) {
            // Map backend field names to frontend form field names
            let fieldName = key;
            if (key === 'admin_type') fieldName = 'adminType';

            // @ts-ignore - dynamic field name
            if (['email', 'password', 'adminType'].includes(fieldName)) {
              // @ts-ignore
              setError(fieldName, {
                type: 'server',
                message: messages[0],
