...
const STATUS_OPTIONS: StatusOption[] = [
  ...
  },
  {
    ...
    label: "Maintenance",
    baseClass:
      "border-accent/40 text-accent hover:bg-accent/10 dark:border-accent/50 dark:text-accent dark:hover:bg-accent/20",
    selectedClass:
      "bg-accent text-accent-foreground border-accent shadow-sm dark:bg-accent dark:border-accent",
    dotClass: "bg-accent",
    selectedDotClass: "bg-white",
  },
];
...
const mapVehicleToModalValues = (vehicle: Vehicle): VehicleModalValues => ({
  ...
  model: vehicle.model ?? "",
  modelDetail: vehicle.model_detail ?? "",
  ...
  registeredYearDate: vehicle.registered_year_date ?? "",
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Type/Body</TableHead>
                    <TableHead>Reg No.</TableHead>
                    <TableHead>Transmission</TableHead>
                    <TableHead>Fuel / Engine</TableHead>
                    ...
                    filteredVehicles.map((vehicle) => (
                      <TableRow key={vehicle.id}>
                        ...
                        <TableCell>
                          ...
                          {vehicle.registered_year_date && (
                            <div className="text-xs text-muted-foreground">Reg: {vehicle.registered_year_date}</div>
                          )}
                        </TableCell>
                        <TableCell className="max-w-[200px]">
                          <div className="text-xs text-muted-foreground truncate" title={vehicle.model_detail}>
                            {vehicle.model_detail || "—"}
                          </div>
                          {vehicle.color && (
                            <div className="text-xs text-muted-foreground mt-1">Col: {vehicle.color}</div>
                          )}
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline">{getVehicleTypeLabel(vehicle.type)}</Badge>
                          {vehicle.body_style && (
                            <div className="text-xs text-muted-foreground mt-1 text-center">{vehicle.body_style}</div>
                          )}
                        </TableCell>
                        ...
                        <TableCell className="text-sm">
                          ...
                          {vehicle.engine_size && (
                            <div className="text-xs text-muted-foreground font-semibold">{vehicle.engine_size}</div>
                          )}
                          {vehicle.euro_status && (
                            <div className="text-[10px] text-muted-foreground">Euro: {vehicle.euro_status}</div>
                          )}
                        </TableCell>
                            ...
                            {STATUS_OPTIONS.map((option) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center gap-2 rounded-md border px-3 py-1 text-xs font-semibold transition-colors ${selected ? option.selectedClass : option.baseClass
                                ...
                                >
                                  <span
                                    className={`h-2.5 w-2.5 rounded-full ${selected ? option.selectedDotClass : option.dotClass
                                      }`}
                                  />
                                  {option.label}
                                </button>
