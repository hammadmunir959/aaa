...
import { MessageSquare, Search, Filter, Eye, RefreshCw, Calendar, ClipboardList, Trash2 } from "lucide-react";
...
const Inquiries = () => {
  ...
  const filteredInquiries = useMemo(() => {
    ...
    setMsgTotal(filtered.length);
    // Slice for current page
    const startIdx = (msgPage - 1) * msgPageSize;
    const endIdx = startIdx + msgPageSize;
    return filtered.slice(startIdx, endIdx);
  }, [inquiries, searchQuery, msgPage, msgPageSize]);

  const getStatusBadge = (status: InquiryStatus) => {
    const label = STATUS_LABELS[status];
    if (status === "unread") {
      return (
        <Badge className="bg-accent/20 text-accent hover:bg-accent/30 border-accent/40">
          {label}
        </Badge>
      );
    }
  ...
  const [prTotal, setPrTotal] = useState<number | null>(null);
  ...
  const filteredPurchaseRequests = useMemo(() => {
    ...
    const filtered = prRequests.filter((request) => {
      const matchesSearch =
        query === "" ||
    ...
    setPrTotal(filtered.length);
    const startIdx = (prPage - 1) * prPageSize;
    const endIdx = startIdx + prPageSize;
    return filtered.slice(startIdx, endIdx);
  }, [prRequests, prSearchQuery, prPage, prPageSize]);
  ...
  const filteredSellRequests = useMemo(() => {
    ...
    const filtered = srRequests.filter((request) => {
      const matchesSearch =
        query === "" ||
        request.name.toLowerCase().includes(query) ||
    ...
    const startIdx = (srPage - 1) * srPageSize;
    const endIdx = startIdx + srPageSize;
    return filtered.slice(startIdx, endIdx);
  }, [srRequests, srSearchQuery, srPage, srPageSize]);
  ...
  const [bkTotal, setBkTotal] = useState<number | null>(null);
  ...
  const filteredBookings = useMemo(() => {
    const filtered = bkClaims.filter((claim) => {
    ...
    setBkTotal(filtered.length);
    const startIdx = (bkPage - 1) * bkPageSize;
    const endIdx = startIdx + bkPageSize;
    return filtered.slice(startIdx, endIdx);
  }, [bkClaims, bkSearchQuery, bkPage, bkPageSize]);
      ...
      <main className="flex-grow py-8">
        <div className="container mx-auto px-4">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Messages, Purchase Requests, Sell Requests, and Claims Inquiries in one place</p>
          </div>
              ...
              <div className="bg-white dark:bg-card shadow rounded-xl p-4">
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-sm text-muted-foreground">
                    {msgTotal !== null ? (
                      <>Showing {(msgPage - 1) * msgPageSize + Math.min(1, filteredInquiries.length)}–{(msgPage - 1) * msgPageSize + filteredInquiries.length} of {msgTotal}</>
                    ) : (
                      <>Showing {filteredInquiries.length}</>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Select value={String(msgPageSize)} onValueChange={(v) => { setMsgPageSize(Number(v)); setMsgPage(1); }}>
                    ...
                    <Button variant="outline" size="sm" onClick={() => setMsgPage((p) => Math.max(1, p - 1))} disabled={msgPage <= 1}>Prev</Button>
                    <div className="text-sm">Page {msgPage}</div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setMsgPage((p) => p + 1)}
                      disabled={msgTotal !== null ? (msgPage * msgPageSize) >= msgTotal : filteredInquiries.length < msgPageSize}
                    >
                      Next
                    </Button>
                  </div>
            ...
            <TabsContent value="purchase-requests" className="space-y-6">
              ...
              <div className="bg-white dark:bg-card shadow rounded-xl p-4">
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button
                                  ...
                                  title="Delete"
                                  onClick={async () => {
                                    if (confirm(`Delete purchase request from ${request.name}?`)) {
                                      try {
                                        await adminCarSalesApi.deletePurchaseRequest(request.id);
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-sm text-muted-foreground">
                    {prTotal !== null ? (
                      <>Showing {(prPage - 1) * prPageSize + Math.min(1, filteredPurchaseRequests.length)}–{(prPage - 1) * prPageSize + filteredPurchaseRequests.length} of {prTotal}</>
                    ) : (
                      <>Showing {filteredPurchaseRequests.length}</>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => setPrPage((p) => Math.max(1, p - 1))} disabled={prPage <= 1}>Prev</Button>
                    <div className="text-sm">Page {prPage}</div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setPrPage((p) => p + 1)}
                      disabled={prTotal !== null ? (prPage * prPageSize) >= prTotal : filteredPurchaseRequests.length < prPageSize}
                    >
                      Next
                    </Button>
                  </div>
            ...
            <TabsContent value="sell-requests" className="space-y-6">
              ...
              <div className="bg-white dark:bg-card shadow rounded-xl p-4">
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button
                                  ...
                                  title="Delete"
                                  onClick={async () => {
                                    if (confirm(`Delete sell request from ${request.name}?`)) {
                                      try {
                                        await adminCarSellRequestsApi.delete(request.id);
                                        toast({ title: "Sell request deleted" });
                                        loadSellRequests();
                                      } catch (error) {
                                        toast({
                                          title: "Delete failed",
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-sm text-muted-foreground">
                    {srTotal !== null ? (
                      <>Showing {(srPage - 1) * srPageSize + Math.min(1, filteredSellRequests.length)}–{(srPage - 1) * srPageSize + filteredSellRequests.length} of {srTotal}</>
                    ) : (
                      <>Showing {filteredSellRequests.length}</>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Select value={String(srPageSize)} onValueChange={(v) => { setSrPageSize(Number(v)); setSrPage(1); }}>
                      <SelectTrigger className="w-28">
                        <SelectValue placeholder="Page size" />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <div className="text-sm">Page {srPage}</div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setSrPage((p) => p + 1)}
                      disabled={srTotal !== null ? (srPage * srPageSize) >= srTotal : filteredSellRequests.length < srPageSize}
                    >
                      Next
                    </Button>
                  </div>
            ...
            <TabsContent value="bookings" className="space-y-6">
              ...
              <div className="bg-white dark:bg-card shadow rounded-xl p-4">
                          ...
                          <TableRow key={claim.id}>
                            ...
                            <TableCell>
                              {claim.status === "pending" ? (
                                <Badge className="bg-accent/20 text-accent hover:bg-accent/30 border-accent/40">Pending</Badge>
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button
                                  ...
                                  title="Delete"
                                  onClick={async () => {
                                    if (confirm(`Delete claim for ${claim.first_name} ${claim.last_name}?`)) {
                                      try {
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-sm text-muted-foreground">
                    {bkTotal !== null ? (
                      <>Showing {(bkPage - 1) * bkPageSize + Math.min(1, filteredBookings.length)}–{(bkPage - 1) * bkPageSize + filteredBookings.length} of {bkTotal}</>
                    ) : (
                      <>Showing {filteredBookings.length}</>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Select value={String(bkPageSize)} onValueChange={(v) => { setBkPageSize(Number(v)); setBkPage(1); }}>
                      <SelectTrigger className="w-28">
                        <SelectValue placeholder="Page size" />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Button variant="outline" size="sm" onClick={() => setBkPage((p) => Math.max(1, p - 1))} disabled={bkPage <= 1}>Prev</Button>
                    <div className="text-sm">Page {bkPage}</div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setBkPage((p) => p + 1)}
                      disabled={bkTotal !== null ? (bkPage * bkPageSize) >= bkTotal : filteredBookings.length < bkPageSize}
                    >
                      Next
                    </Button>
                  </div>
          ...
          {selectedInquiry && (
            <div className="grid gap-4 py-4">
              ...
              <div>
                ...
                <p className="text-base text-gray-700 dark:text-gray-300 whitespace-pre-wrap bg-gray-50 dark:bg-black p-3 rounded-md">
