...
export const chatbotApi = {
  ...
  async deleteConversation(id: number): Promise<void> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || "Unable to delete conversation.");
    }
  },
  ...
  async getLatestMessages(sessionId: string, lastMessageId?: number): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        // Throw error for rate limiting so frontend can handle it
        if (response.status === 429) {
          const error: any = new Error('Rate limit exceeded');
          error.status = 429;
          throw error;
        }
        // For other errors, return empty (silent fail for polling)
