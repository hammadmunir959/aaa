...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = type('StandardPagination', (PageNumberPagination,), {'page_size': 10, 'page_size_query_param': 'page_size', 'max_page_size': 100})
    ...
    def get_queryset(self):
        ...
        if self.action in {'retrieve', 'send_manual_reply', 'toggle_manual_reply'}:
            return queryset.prefetch_related(
                Prefetch('messages', queryset=ConversationMessage.objects.order_by('timestamp'))
            )
...
def chatbot_message(request):
    ...
    async def process_async_logic(msg_data, client_ip):
        try:
            ...
            logger.info(f"Chatbot: Start processing message for session {session_id} from {client_ip}")

            if not user_message or not session_id:
                logger.warning(f"Chatbot: Missing message or session_id for {client_ip}")
            ...
            try:
                ...
                logger.info(f"Chatbot: Conversation {'created' if created else 'retrieved'} (ID: {conversation.id})")
            except AttributeError:
                logger.info("Chatbot: aget_or_create not available, using sync_to_async fallback")
                from asgiref.sync import sync_to_async
            ...
            if created:
                ...
                def cleanup_others():
                    count = Conversation.objects.filter(
                        ip_address=client_ip,
                        status='active'
                    ).exclude(id=conversation.id).update(
                        status='completed',
                ...
                logger.info(f"Chatbot: Cleaned up {count} other active sessions for {client_ip}")
            ...
            if conversation.manual_reply_active:
                 logger.info(f"Chatbot: Manual reply active for session {session_id}, skipping AI")
                 return Response({
                     'message': '',
                     'response_time_ms': 0,
                     'session_id': session_id,
            ...
            ai_response = await service.process_message_async(user_message, conversation)
            logger.info(f"Chatbot: AI response generated in {ai_response.get('response_time_ms')}ms")
            ...
            return Response({
                ...
                'session_id': session_id,
                'manual_reply_active': False,
                'contact_info_collected': ai_response.get('contact_info_collected', False),
