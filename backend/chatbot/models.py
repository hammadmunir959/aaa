...
class Conversation(models.Model):
    ...
    def check_and_mark_completed(self):
        ...
        if self.last_activity and (timezone.now() - self.last_activity) > timedelta(minutes=2):
            self.status = 'completed'
            self.ended_at = timezone.now()
...
class ContactInfo(models.Model):
    ...
    source_message = models.TextField(blank=True, help_text="Original message that contained this contact info")
...
class ContentIndex(models.Model):
    ...
    content_object = models.CharField(max_length=200, help_text="Model instance identifier")
    ...
    class Meta:
        ...
        indexes = [
            ...
            GinIndex(fields=['content_text'], name='contentindex_content_text_gin', opclasses=['gin_trgm_ops']),
            GinIndex(fields=['keywords'], name='contentindex_keywords_gin', opclasses=['gin_trgm_ops']),
        ]
