...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting content indexing for chatbot knowledge base...')
        )
        ...
        if options['clear']:
            ...
            self.stdout.write(
                self.style.WARNING(f'🗑️  Cleared {deleted_count} existing content index entries')
            )
        ...
        searchable_content = ContentIndex.objects.filter(is_active=True, is_searchable=True).count()

        content_types = ContentIndex.objects.values('content_type').annotate(
