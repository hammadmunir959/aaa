...
class CarPurchaseRequestViewSet(viewsets.ModelViewSet):
    queryset = CarPurchaseRequest.objects.select_related(
        "car_listing", "assigned_staff"
    )
...
class CarSellRequestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            response_serializer = CarSellRequestSerializer(
                sell_request, context={"request": request}
            )
            return Response(response_serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
