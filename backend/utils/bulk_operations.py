...
def bulk_update_optimized(
    objects: List[ModelType], fields: List[str], batch_size: int = 100
) -> int:
    """
    ...
    with transaction.atomic():
        for i in range(0, len(objects), batch_size):
            ...
            updated = models.Model.objects.bulk_update(
                batch, fields, batch_size=batch_size
            )
...
def bulk_delete_optimized(queryset: QuerySet[ModelType], batch_size: int = 100) -> int:
    """
...
def bulk_update_fields(
    queryset: QuerySet[ModelType], updates: Dict[str, Any], batch_size: int = 100
) -> int:
    """
    ...
    with transaction.atomic():
        for i in range(0, count, batch_size):
            ...
            model.objects.bulk_update(
                batch, list(updates.keys()), batch_size=batch_size
            )
            updated_count += len(batch)
