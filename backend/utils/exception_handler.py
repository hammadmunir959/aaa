...
def custom_exception_handler(exc: Exception, context: dict[str, Any]) -> Any:
    ...
    if isinstance(exc, DjangoValidationError):
        ...
        if hasattr(exc, "error_dict"):
            errors = {
                k: [str(v) for v in v_list] for k, v_list in exc.error_dict.items()
            }
    ...
    if isinstance(exc, DRFValidationError):
        ...
        if hasattr(exc, "detail"):
            if isinstance(exc.detail, dict):
                errors = {
                    k: v if isinstance(v, list) else [v] for k, v in exc.detail.items()
                }
    ...
    if isinstance(exc, APIException):
        if response is not None:
            ...
            if isinstance(error_data, dict):
                ...
                message = (
                    error_data[0] if isinstance(error_data, list) else str(error_data)
                )
