...
def optimize_image(
    ...
    file_extension = (
        ".webp" if format == "WEBP" else (".png" if format == "PNG" else ".jpg")
    )
...
def create_thumbnail(
    image_file, size: Tuple[int, int] = (300, 300), quality: int = 75
) -> InMemoryUploadedFile:
    """
    ...
    try:
        ...
        if max_size:
            ...
            if width > max_size[0] or height > max_size[1]:
                return (
                    False,
                    f"Image dimensions ({width}x{height}) exceed maximum ({max_size[0]}x{max_size[1]})",
                )
        ...
        if max_file_size:
            ...
            if file_size > max_file_size:
                return (
                    False,
                    f"Image size ({file_size} bytes) exceeds maximum ({max_file_size} bytes)",
                )
