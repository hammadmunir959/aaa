        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if cached_packet is not None:
                    ...
                    if isinstance(cached_packet, dict) and 'data' in cached_packet:
                        ...
                        response = Response(data=cached_packet['data'], status=cached_packet.get('status', 200))
            ...
            if hasattr(response, 'status_code') and response.status_code == 200:
                try:
                    ...
                    logger.debug(f"Cached API response for {cache_key} (TTL: {timeout}s)")
                except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if keys:
                ...
                logger.info(f"Invalidated {deleted} cache keys matching '{key_pattern}' (Redis)")
                return deleted
            ...
            if hasattr(cache, '_cache'):
                formatted_pattern = key_pattern.replace('*', '') # Simple contain check for fallback
                keys_to_delete = []
                ...
                if keys_to_delete:
                    for k in keys_to_delete:
                        cache.delete(k)
                    logger.info(f"Invalidated {len(keys_to_delete)} cache keys matching '{key_pattern}' (LocMem)")
                    return len(keys_to_delete)
            
            # If we reached here, we couldn't invalidate
            logger.debug(f"Could not invalidate pattern '{key_pattern}' - Backend not supported or Redis failed: {redis_error}")
            return 0
