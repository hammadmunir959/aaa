    ...
    def validate(cls, is_production: bool = False) -> tuple[bool, List[str]]:
        ...
        if is_production:
            for var in cls.REQUIRED_PROD_VARS:
                ...
                elif var in cls.NO_DEFAULT_PROD_VARS and value.startswith(
                    ("your-", "default-", "change-")
                ):
                    errors.append(
        ...
        if is_production:
            ...
            if secret_key and len(secret_key) < 50:
                warnings.append(
                    "Consider using a longer SECRET_KEY (at least 50 characters). "
                    "Current key is shorter than 50 characters."
                )
        ...
        if is_production and not db_name:
            warnings.append(
                "Consider using a more specific database name in production"
            )
