...
class Command(BaseCommand):
        ...
        try:
            if options["database"]:
                ...
                self._handle_cleanup(
                    options["cleanup"], dry_run=options["cleanup_dry_run"]
                )
    ...
    def _handle_verify_backup(self, backup_path):
        ...
        if verify_backup_integrity(backup_path):
            self.stdout.write(self.style.SUCCESS("✓ Backup verification successful"))
        else:
            self.stdout.write(
    ...
    def _handle_cleanup(self, days, dry_run=False):
        ...
        if dry_run:
            ...
            for backup in old_backups:
                self.stdout.write(
                    f'  - {backup["filename"]} ({backup["created"].strftime("%Y-%m-%d %H:%M:%S")})'
                )
        else:
            deleted_count = cleanup_old_backups(days)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Cleanup completed: {deleted_count} backup(s) removed"
                )
            )
