...
def _build_device_case() -> Case:
    ...
    return Case(
        When(Q(user_agent__isnull=True) | Q(user_agent__exact=''), then=Value('Unknown')),
...
def _build_dashboard_summary() -> dict:
    ...
    authenticated_today = VisitorSession.objects.filter(
        last_activity__date=today,
        user__isnull=False
    ).values('user_id').distinct().count()
    
    anonymous_today = VisitorSession.objects.filter(
        last_activity__date=today,
        user__isnull=True
    ...
    authenticated_week = VisitorSession.objects.filter(
        last_activity__gte=week_ago,
        user__isnull=False
    ).values('user_id').distinct().count()
    
    anonymous_week = VisitorSession.objects.filter(
        last_activity__gte=week_ago,
        user__isnull=True
    ).exclude(session_id="").values('session_id').distinct().count()
    ...
    authenticated_month = VisitorSession.objects.filter(
        last_activity__gte=month_ago,
        user__isnull=False
    ).values('user_id').distinct().count()
    
    anonymous_month = VisitorSession.objects.filter(
        last_activity__gte=month_ago,
        user__isnull=True
    ).exclude(session_id="").values('session_id').distinct().count()
...
def notifications(request):
    ...
    if not request.query_params.get('page'):
        serializer = NotificationSerializer(queryset[:50], many=True)  # Limit to 50 for dropdown
        return Response(serializer.data)
...
def web_analytics_overview(request):
    ...
    authenticated_visitors = VisitorSession.objects.filter(
        last_activity__gte=start,
        user__isnull=False
    ).values('user_id').distinct().count()
    
    anonymous_visitors = VisitorSession.objects.filter(
        last_activity__gte=start,
        user__isnull=True
    ).exclude(session_id="").values('session_id').distinct().count()
    ...
    avg_session_duration = valid_sessions_qs.filter(
    ...
    total_duration = valid_sessions_qs.filter(
        duration_seconds__isnull=False,
        duration_seconds__lt=86400  # Exclude sessions > 24 hours (legacy data anomalies)
    ).aggregate(
    ...
    avg_pages_per_session = valid_sessions_qs.aggregate(
    ...
    daily_pageviews_lookup = {entry['day']: entry['views'] for entry in raw_daily_pageviews}
    ...
    for entry in raw_daily_authenticated:
        day = entry['day']
        daily_visitors_lookup[day] = daily_visitors_lookup.get(day, 0) + entry['visitors']
    ...
    source_counts = (
        pageviews_qs
        .annotate(source=_build_referrer_case())
        .values('source')
        .annotate(views=Count('id'))
        .order_by('-views')
    )
    ...
    device_counts = (
        pageviews_qs
        .annotate(device=_build_device_case())
        .values('device')
        .annotate(views=Count('id'))
        .order_by('-views')
    )
...
def fleet_status(request):
    ...
    status_counts = Vehicle.objects.values('status').annotate(
        count=Count('id')
    )
...
def sales_pipeline(request):
    ...
    active_statuses = ['pending', 'contacted', 'viewing_scheduled', 'offer_made', 'accepted']
    
    pipeline_data = CarPurchaseRequest.objects.filter(
        status__in=active_statuses,
        offer_price__isnull=False
    ).aggregate(
        total_value=Sum('offer_price'),
        count=Count('id')
    )
...
def upcoming_returns(request):
    ...
    returns = Claim.objects.filter(
        status='approved',
        end_date__gte=today,
        end_date__lte=three_days_later
    ).select_related('vehicle').order_by('end_date')[:5]
