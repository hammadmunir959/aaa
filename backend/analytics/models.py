...
class ActivityLog(models.Model):
    ...
    user = models.ForeignKey('accounts.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='activities')
    recipient = models.ForeignKey('accounts.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='notifications', help_text='User who should be notified about this activity')
    ...
    notification_type = models.CharField(max_length=20, choices=NOTIFICATION_TYPES, default='info', help_text='Visual type for notification display')
    priority = models.CharField(max_length=20, choices=PRIORITY_LEVELS, default='medium', help_text='Priority level of the notification')
    action_url = models.CharField(max_length=500, blank=True, help_text='URL to navigate to when notification is clicked')
    expires_at = models.DateTimeField(null=True, blank=True, help_text='Optional expiration date for the notification')
