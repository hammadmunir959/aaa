...
from utils.tasks import run_full_backup, run_database_backup, run_media_backup, cleanup_expired_backups
...
def backup_stats(request):
    ...
    try:
        ...
        return Response(
            {'error': 'Failed to retrieve backup statistics', 'details': str(e) if settings.DEBUG else None},
...
def backup_list(request):
    ...
    try:
        ...
        logger.info(f"Backup list retrieved by user {request.user.username}, found {len(backups)} backups")
        ...
        return Response(
            {'error': 'Failed to list backups', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def backup_create(request):
    ...
    try:
        ...
        if backup_type == 'database':
            ...
            logger.info(f"Database backup task queued by user {request.user.username}, task_id: {result.id}")
            return Response({
            ...
            logger.info(f"Media backup task queued by user {request.user.username}, task_id: {result.id}")
            return Response({
            ...
            logger.info(f"Full backup task queued by user {request.user.username}, task_id: {result.id}")
            return Response({
        ...
        return Response(
            {'error': f'Failed to create backup: {str(e)}', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def backup_verify(request):
    ...
    try:
        ...
        logger.info(f"Backup verification by user {request.user.username} for {backup_path}: {'valid' if is_valid else 'invalid'}")
        return Response({
            'path': backup_path,
            'valid': is_valid,
        ...
        return Response(
            {'error': f'Failed to verify backup: {str(e)}', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def backup_cleanup(request):
    ...
    try:
        ...
        logger.info(f"Backup cleanup by user {request.user.username}: deleted {deleted_count} backups older than {days} days")
        return Response({
        ...
        return Response(
            {'error': f'Failed to cleanup backups: {str(e)}', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def backup_config(request):
    ...
    try:
        ...
        config = {
            ...
            'backup_directory': str(getattr(settings, 'DBBACKUP_BACKUP_DIRECTORY', '/opt/backups/pchm')),
        }
        return Response(config, status=status.HTTP_200_OK)
    except Exception as e:
        logger.error(f"Error getting backup config: {e}", exc_info=True)
        return Response(
            {'error': 'Failed to retrieve backup configuration', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def backup_task_status(request, task_id):
    ...
    try:
        ...
        return Response(
            {'error': 'Failed to get task status', 'details': str(e) if settings.DEBUG else None},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
