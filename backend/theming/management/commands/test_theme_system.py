...
class Command(BaseCommand):
    ...
    def _test_api_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.stdout.write(self.style.SUCCESS(f"  ✓ API endpoint accessible"))
                self.stdout.write(f"    Theme: {data.get('theme_key', 'unknown')}")
                if data.get("event"):
                    self.stdout.write(
    ...
    def _test_theme_resolution(self):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"  ✓ Theme resolution working"))
    ...
    def _test_events(self):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"  ✓ Events query working"))
            ...
            self.stdout.write(self.style.ERROR(f"  ✗ Events test failed: {str(e)}"))
    ...
    def _test_cache(self):
        ...
        try:
            ...
            if cached is not None:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  ✓ Cache accessible (empty, will populate on next request)"
                    )
                )
        except Exception as e:
