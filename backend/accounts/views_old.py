...
def register_admin(request):
    ...
    with transaction.atomic():
        ...
        if admin_type == User.ROLE_SUPER_ADMIN and super_admin_exists:
            return JsonResponse(
                {
                    "error": "A Super Admin already exists. Only one is allowed.",
        ...
        if admin_type == User.ROLE_ADMIN and not super_admin_exists:
            return JsonResponse(
                {
                    "error": "A Super Admin must be created before registering admins.",
        ...
        status_value = (
            User.STATUS_ACTIVE
            if admin_type == User.ROLE_SUPER_ADMIN
            else User.STATUS_PENDING
        )
...
def login_admin(request):
    ...
    if not email:
        ...
        SecurityAuditLogger.log_failed_login(
            email_raw, ip_address, "Invalid email format"
        )
        return error_response(
    ...
    if user.status != User.STATUS_ACTIVE:
        SecurityAuditLogger.log_failed_login(
            email, ip_address, f"Account not active: {user.status}"
        )
        return error_response(
...
def verify_otp(request):
    ...
    if user and purpose == "verification":
        ...
        if not was_verified:
            ...
            if (
                user.admin_type == User.ROLE_ADMIN
                and user.status == User.STATUS_PENDING
            ):
...
def list_admin_requests(request):
    ...
    admins = User.objects.filter(admin_type=User.ROLE_ADMIN, is_email_verified=True)
    if status_filter:
