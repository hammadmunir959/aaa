...
def subscribe_newsletter(request):
    ...
    subscriber, created = NewsletterSubscriber.objects.get_or_create(
        email=email,
        defaults={"name": name, "source": request.data.get("source", "website")},
    )

    if created:
        # Send welcome email
        html_message = render_to_string(
            "emails/newsletter_welcome.html", {"name": name or "there", "email": email}
        )
        plain_message = f"Welcome to AAA Accident Solutions LTD newsletter! You've successfully subscribed with {email}."
        ...
        return Response(
            {"message": "Email already subscribed"}, status=status.HTTP_200_OK
        )
...
class NewsletterCampaignViewSet(viewsets.ModelViewSet):
    ...
    def send_campaign(self, request, pk=None):
        ...
        try:
            # Custom connection to authenticate with noreply but send as news
            ...
            # Send from noreply (authenticated account) with reply-to news (alias)
            NEWSLETTER_FROM_EMAIL = "AAA Accident Solutions LTD <noreply@aaa-as.co.uk>"
            NEWSLETTER_REPLY_TO = "info@aaa-as.co.uk"
            ...
            # Open connection once for mass sending
            connection.open()
            ...
            for email in recipients:
                try:
                    ...
                    recipient, _ = NewsletterRecipient.objects.get_or_create(
                        ...
                        defaults={
                            "is_test": False,
                            "token": signer.sign(f"{campaign.id}:{email}"),
                        },
                    )
                    ...
                    html_content = re.sub(
                        r'href="([^"]+)"', _rewrite_link, html_content
                    )
    ...
    def send_test(self, request, pk=None):
        ...
        try:
            ...
            recipient, _ = NewsletterRecipient.objects.get_or_create(
                ...
                defaults={
                    "is_test": True,
                    "token": signer.sign(f"{campaign.id}:{email}:test"),
                },
            )
            if not recipient.token:
            ...
            # Custom connection for test email
            connection = get_connection(
                host=settings.EMAIL_HOST,
                port=settings.EMAIL_PORT,
                username=settings.EMAIL_HOST_USER,
    ...
    if token:
        try:
            ...
            if recipient.open_count == 0 and not recipient.is_test:
                NewsletterCampaign.objects.filter(pk=pk).update(
            ...
            recipient.save(
                update_fields=["open_count", "first_opened_at", "last_opened_at"]
            )
...
def campaign_click_redirect(request, pk: int):
    ...
    if not target:
        return Response(
            {"error": "Missing target url"}, status=status.HTTP_400_BAD_REQUEST
        )
    if token:
        try:
            recipient = NewsletterRecipient.objects.get(campaign_id=pk, token=token)
            ...
            recipient.save(
                update_fields=["click_count", "first_clicked_at", "last_clicked_at"]
            )
        except NewsletterRecipient.DoesNotExist:
            pass
